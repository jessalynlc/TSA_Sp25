...
> Comments: This is by far the best fit. Notice the residual series seems to be random and ACF shows no significant self correlation. Doesn't seem to have long memory here (based on time series), most ACF values are within the blue dashed lines which is what we want, and looking at the histogram, this looks much more like a normal distribution than the other two.

This has been backward assessment.
...
#ACF1 was not calculated because we're only providing 12 observations, not enough to find ACF1
...
```{r}
autoplot(MEAN_seas, series = "Mean") +
  autolayer(SNAIVE_seas, series = "Naive")+
  autolayer(SARIMA_for, series = "SARIMA")
