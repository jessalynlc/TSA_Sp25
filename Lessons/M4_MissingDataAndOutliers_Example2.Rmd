...
```{r}
ts_wind_data <- ts(wind_data$AWND.1, start = c(1984, 1), frequency=12) 

autoplot(ts_wind_data) +
  ylab("Average Wind Speed") 

#this tells us how many NAs we have
summary(wind_data$AWND.1)
```

Note that there are gaps in the autoplot where data is missing! But sometimes if you have too many observations, you might not be able to see them.
...
```{r}
library(zoo)
clean_wind_data <- na.approx(ts_wind_data) 
...
```{r}
clean_wind_data <- na.interp(ts_wind_data)
