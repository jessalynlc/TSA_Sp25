...
```{r}
#Create vector t
t <- c(1:nrow(inflow_data)) 

#Fit a linear trend to TS of iHP
data_HP1 <- data.frame("t" = t, "inflowHP1" = inflow_data$HP1)
linear_trend_HP1 = lm(inflowHP1~t, data_HP1)
summary(linear_trend_HP1)
```
...
#grabbing intercept term (beta0)
beta0 = as.numeric(linear_trend_HP1$coefficients[1])

#grabbing slope term (beta1)
beta1 = as.numeric(linear_trend_HP1$coefficients[2])

#create linear trend series
linear_trend <- (beta0 + beta1*t)
detrend_inflow <-  inflow_data[,2] - linear_trend
ts_detrend <- ts(detrend_inflow, start = c(1931,1), frequency = 12)

#visualize trend on observed data
autoplot(ts_inflow_data[,1])+
  autolayer(ts_detrend, series="Series Detrended")
```
...
seas_means_model = lm(ts_detrend~dummies)
