...
-   ETS
-   ARIMA with dynamic harmonic fourier components (harmonic fourier components are sinusoidal curves, which help represent seasonal patternsß)
-   TBATS
-   Neural Network (help with modeling non-linear relationships)
...
ts_act_power_hourly <- msts(act_power_hourly$hourly_mean_active_power, 
                            seasonal.periods =c(24,168,8766), #day hours, week hours, year hours
...
Decompose: would use ts.frequency argument to create seasonal component. Only seasonal component with respect to max seasonal period would be represented. You can still do it, it will just default to higher frequency though. Want to make sure we're creating patterns for each one of our components htough. stl() can however handle msts objects.
...
If you see trend in residuals, you might be forcing seasonality that you don't need.
...
ts_act_power_daily_train <- subset(ts_act_power_daily,
                                   end = length(ts_act_power_daily)-n_for) #stops 365 days before

#create a subset for testing purpose
ts_act_power_daily_test <- subset(ts_act_power_daily,
                                   start = length(ts_act_power_daily)-n_for) #last 365 observations
...
The model used with `stlf` is a ETS(.,.,.) model. (decomposes AND creates forecasts)
...
```
There is a deep dip after 2009, might be trying to mimic a previous year.
Another problem here is that it's generating some negative values, and that can happen sometimes because when you're forecasting future observations, you might be generating negative numbers (in the CIs) draw from truncated observations is one way to help
...
NN_for <- forecast(NN_fit, 
                   h=365,
>>>>>>> 9d7415bdc0f4de805246d6a013100ba684ba5b49
                   xreg=fourier(ts_act_power_daily_train, 
                                          K=c(2,12),h=365))
