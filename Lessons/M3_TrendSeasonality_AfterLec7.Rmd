...
#take time series, which is just a column vector, we org into a matrix with columns representing months in a way we can just find average of a column/row so we can get yearly averages instead of monthly averages
inflow_data_matrix <- matrix(ts_inflow_data[,iHP], byrow = FALSE, nrow = 12) 
inflow_data_yearly <- colMeans(inflow_data_matrix)

my_year <- c(1931:2010)
inflow_data_yearly <- data.frame(my_year, "inflow_data" = inflow_data_yearly)

ggplot(inflow_data_yearly, aes(x=my_year, y=inflow_data))+
  geom_line(color="blue")+
...
summary(SeasonalMannKendall(ts_inflow_data[,iHP]))

#Use yearly date to run Mann Kendall
#need to add summary to show results printed out
summary(MannKendall(inflow_data_yearly$inflow_data))

library(trend)
summary(smk.test(ts_inflow_data[,iHP]))
```

reject the null hypothesis since pvalue is less than 0.05. Time series follows a trend, and that matches what we saw

Seasonal MK tests same hypthoesis, calculates score slightly differently, but we got the same results as when we subtracted the seasonal component and just did the Mann Kendall!

Output from smk.test is more informative to see if there is an inc or dec trend in each season of the yr. We might have one season with inc trend and one with dec trend, and that might average out for outer Seasonal MK test, but this one can show you month breakdown
...
print(cor.test(inflow_data_yearly$inflow_data, my_year, method = "spearman"))

print(cor.test(inflow_data_yearly$inflow_data, my_year, method = "pearson"))
```
...
print(adf.test(ts_inflow_data[,iHP], alternative = "stationary")) #stationary over stochastic trend
#Now let's try the yearly data
print("Results for ADF test on yearly data/n")
print(adf.test(inflow_data_yearly$inflow_data, alternative = "stationary")) 
```
