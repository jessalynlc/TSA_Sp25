...
```{r}
SSES_seas <- es(ts_price,model="ZZZ",h=n_for,holdout=FALSE) #holdout = FALSE bc we have separated time series objects into train and test, so don't need to hold out on anything
plot(SSES_seas)
...
Don't think there is a version of StructTS() that can be used for granular data (you would need gammas for every single "season" and there would be a lot)

```{r}
SS_seas <- StructTS(ts_price,
                    #BSM because we need seasonal term, there are 4 terms and they correspond to standard deviations: equation related to levels, equation related to slope, equation related to seasonal terms, equation related to y_t
                    type="BSM",fixed=c(NA,NA,NA,NA))   #this function has convergence issues
checkresiduals(SS_seas)
#from residuals, this is not a good fit, we still see some seasonality in early years, ACF shows some temporal dependence here that could be result of not a good fit for the rend term and not a good fit for seasonal term
#We should now try to specify variances to get a better model
```

```{r}
SS_seas <- StructTS(ts_price,
                    #assigning these terms tends to be trial and error! If you made one of the standard deviations 0, go back to what we had before changing 0.3 you are only messing with posterior distribution
                    type="BSM",fixed=c(0.1,0.01,0.3,NA)) 

#this function has convergence issues
checkresiduals(SS_seas)
#still not ideal, but better than the last one

#Generating forecasts
# StructTS() does not call the forecast() internally so we need one more step
SS_for <- forecast(SS_seas,h=n_for)
plot(SS_for)
#failed to represent the trend and seasonality!
```
...
These are errors based on the mean of the probability distribution functions, they don't take into account confidence intervals, just the blue curve. We can see that the BSM and SSES were performing better than the other three models.
