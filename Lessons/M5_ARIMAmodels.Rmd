...
#Prices are decreasing, so what's happening?
...
ACF is slowly decaying over time, so we are working with a process that carries info from one to another. PACF: we only see one significant spike at lag 1, indicating this could be an AR1 model.
When we look at coefficient for lag 1, it's very close to lag 1, so might be case we need to check for unit 1. We have decreases in ACF, but when we get close to seasonal lags, it seems to increase a little bit, giving us indication that there is some seasonality happenign in electricity price as well.
...
My notes: To fit an ARIMA model in a dataset, we dont know yet how to handle seasonality. But what you do is take out seasonality out of the dataset before building AR/MA or ARIMA on this dataset.

Luana's notes: To take seasonality only out of the data set, we will use function seasadj() from package forecast. The function returns seasonally adjusted data constructed by removing the seasonal component. It takes one main object that should be created using decompose() function.

my notes:
When you fit linear/seasonal means model to your data, you can subtract those from your original model. It's the same idea here. If you look at decompose object, we have each of the comps separately, all you have to do is get x, your original series and subtract seasonal. But there is a function that does it for you already!
...
#object has to be decomposed object
...
#We didn't change the series, but we just fixed some of the movements due to seasonality
...
#Again we have some spikes here close to seasonal lags, but after removing seasonality, we just have one significance falling outside of blue lines!
```
...
#Run ADF to make sure you don't have stochastic trend since ARIMA can only run with stationary series
print(adf.test(deseasonal_residential_price, alternative = "stationary"))
#pvalue is 0.9551, this is greater than 0.05, which means we accept null that the data has a unit root, in other words there is a stochastic trend. In order to fit ARIMA model, we have to differentiate the series
...
#Could be ARMA where one characteristic is superimposed on another
```
...
print(Model_111)
#y_t = 0.030 + .6312*y_t-1 - 0.7426*a_t-1 + a_t 
#AIC you have original one and AICc which is corrected for when you have small sample sizes, and you have Bayesian information criteria. The lower the AIC, the better your ARIMA fit

#comparing data frame that will have all your AIC values! As you try more models, store, then in the end choose model that has the lowest one
compare_aic <- data.frame(Model_111$aic)

#Check residuals series, if white noise we got a good fit (no trends, no seasonality, nothing happen with error term. You did a good job modeling patterns in your series.)
plot_grid(
  autoplot(Model_111$residuals), #doesn't seem to follow any trends or seasonality!
  #you have most autocorrelation values within blue line, which is what you want
  autoplot(Acf(Model_111$residuals,lag.max=40, plot = FALSE)),
  #very similar behavior most coefficients are within two dashed lines
  autoplot(Pacf(Model_111$residuals,lag.max=40, plot=FALSE)), nrow = 1
)
...
plot_grid(
  ...
  autoplot(Pacf(Model_011$residuals,lag.max=40, plot=FALSE)), nrow = 1
)
...
plot_grid(
  ...
  autoplot(Pacf(Model_211$residuals,lag.max=40, plot=FALSE)), nrow = 1
)
...
Now that you have played with different order, let's try the auto.arima() function from the base package *stats*. The best fit for this time series is an ARIMA(0,1,2) with drift.
