...
```{r}
ts_inflow_data <- ts(inflow_data[,2:4], start =c(1931,1), frequency = 12) #need to exclude date column or else it will try to convert it as well! Frequency helps to model seasonality of dataset here. If you have daily data, frequency would be 365 because after 365 days you would be starting over at Jan 1st
ts_inflow_data
...
#gave acf function your first column, then ask the function to plot 40 lags
HP1_acf=Acf(ts_inflow_data[,1], lag = 60)
#First bar represents correlation at lag 0, calculating correlation between t and t (ie dec and dec, or nov and nov, month with itself)
#Second bar is close to 0.8, this is telling you that correlation at lag 1 is between .8 and .9. Relationship bw month and month before is 0.8.
#acf cyclical means that data has strong seasonal component
#lag 60 is relationship between today and 60 months ago
#lag 12 is relationship between today and 12 months ago
#case sensitive acf vs Acf, capital shows lag correctly but does not start at 0

HP1_pacf= Pacf(ts_inflow_data[,1], lag = 60)
#notice first bar is the second bar in the acf
