...
energy_subset <- energy_data[, c(1, 4:6)]
head(energy_subset)
energy_subset$Month <- ymd(energy_subset$Month)
```
...
```{r}
energy_subset_ts <- ts(energy_subset[,2:4],start=c(1973,1),frequency=12)
```
...
autoplot(energy_subset_ts) +
  labs(
    ...
    color = "Renewable Energy Production/Consumption Source"
  ) +
...
The correlation between Total Renewable Energy Production and Hydroelectric Power Consumption is very weak (r = -0.0292) and not statistically significant (p = 0.4682). This suuggests that there is no meaningful relationship between these two variables.
