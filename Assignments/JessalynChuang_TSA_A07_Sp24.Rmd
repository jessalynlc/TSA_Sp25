...
It wouldn't be fair to compare the models by comparing the residuals as it wouldn't be fair to compare the two models in the first place as they are taking in different input data. The ARIMA ran on deseasoned data while the SARIMA ran on the original data.

Evaluating the SARIMA model alone, it appears to capture underlying patterns well because its residuals do not appear very autocorrelated and look to be more randomly distributed.
